...
im_stat = pd.read_csv(
    "../CheXpert-v1.0-small/image_statistics.csv",
    names=["mean_R", "mean_G", "mean_B", "std_R", "std_G", "std_B"])

# Normalization Step
chexpert_mean_color = [
    im_stat["mean_R"][0], im_stat["mean_G"][0], im_stat["mean_B"][0]
]
# The Standard Deviation Across the Training Images
chexpert_std_color = [
    im_stat["std_R"][0], im_stat["std_G"][0], im_stat["std_B"][0]
]
...
transform = transforms.Compose([
    ...
    transforms.ToTensor(),
    pretrained_normalize if pretrained else transforms.Normalize(
        chexpert_mean_color, chexpert_std_color),
])
...
for epoch in range(NUM_EPOCHS):
    train_loss, train_auc_lb = train(model, device, train_loader, criterion,
                                     optimizer, epoch)
    valid_loss, valid_auc_lb, valid_results = evaluate(model, device,
    ...
    is_best = np.mean(
        (valid_auc_lb)
    ) > best_mean_auc  # let's keep the model that has the best avg. AUC, but you can also use another metric.
    if is_best:
...
test_loss, test_auc_lb, test_results = evaluate(
    best_model, device, test_loader, criterion, print_freq=1)
r = np.array(test_results)
pc_f1 = [
    f1_score(r[:, 0, label], r[:, 1, label])
    for label in np.arange(r.shape[2])
]
