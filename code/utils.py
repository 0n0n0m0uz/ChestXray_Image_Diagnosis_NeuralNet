...
def compute_batch_avg_auc(output, target):
    ...
    with torch.no_grad():
        ...
        # per class AUC
        pc_output_np_lb = np.hstack((pc_output_np_lb,
                                     [[1, 0]] * target_np.shape[1]))
        pc_target_np_lb = np.hstack((pc_target_np_lb,
                                     [[0, 1]] * target_np.shape[1]))
        ...
        pc_correction_lb = pc_target_np_lb.shape[1] * 1.0 / (
            pc_target_np_lb.shape[1] - 2)

        pc_lb = [
            pc_correction_lb * roc_auc_score(t, o, 'weighted')
            for t, o in zip(pc_target_np_lb, pc_output_np_lb)
        ]
...
def train(model,
    ...
    for i, (input, target) in enumerate(data_loader):
        ...
        if isinstance(input, tuple):
            input = tuple([
                e.to(device) if type(e) == torch.Tensor else e for e in input
            ])
        else:
...
def make_kaggle_submission(list_id, list_prob, path):
    if len(list_id) != len(list_prob):
        raise AttributeError(
            "ID list and Probability list have different lengths")
